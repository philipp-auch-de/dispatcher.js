...
export function dateToIsoString(date) {
    ...
    return date.getFullYear() + '-' + pad(date.getMonth() + 1, 2) + '-' + pad(date.getDate(), 2) + 'T' +
        pad(date.getHours(), 2) + ':' + pad(date.getMinutes(), 2) + ':' + pad(date.getSeconds(), 2) + dif +
        pad(Math.floor(Math.abs(tzo) / 60), 2) + ':' + pad(Math.abs(tzo) % 60, 2);
}
...
export function getDateByDayAndSetPos(date, day, setPos) {
...
export function getDateFromString(input) {
    ...
    if (!isValidDate(res)) {
        ...
        res = new Date(input.substring(0, 4) + '-' + input.substring(4, 6) + '-' + input.substring(6, 11) + ':' +
                           input.substring(11, 13) + ':' + input.substring(13, 15));
    }
    if (!isValidDate(res)) {
...
export function getStringFromDate(date) {
    return '' + (1900 + date.getYear()) + pad(date.getMonth() + 1, 2) + pad(date.getDate(), 2) + 'T' +
        pad(date.getHours() - 2, 2) + pad(date.getMinutes(), 2) + pad(date.getSeconds(), 2);
}
